...
<?php echo $text_phone?>
<input type="text" name="phone" id="phone" value="<?php echo $phone;?>"> <?php echo $text_help?><br>
<br />
...
$('#button-confirm').bind('click', function() {
	$.ajax({ 
		type: 'POST',
		data: 'pin=' + $('#pin').val(),
		url: 'index.php?route=module/jossmsforgotpwdverify/confirm',
...
$('#button-startver').bind('click', function() {
	...
	if (!wait) {
		...
		$.ajax({ 
			...
			url: 'index.php?route=module/jossmsforgotpwdverify/start',
			success: function(data) {
			wait = 0;
				switch (data) {
				case "1":
