...
<?php $kuler->addScript('catalog/view/javascript/jquery/jquery.jcarousel.min.js') ?>
<?php echo $header; ?>
			...
			<?php if ($thumb || $description) { ?>
				<div class="category-info row">
					<?php if ($thumb && !$kuler->getSkinOption('hide_category_image')) { ?>
						<div class="image col-md-4"><img src="<?php echo $thumb; ?>" alt="<?php echo $heading_title; ?>" /></div>
					<?php } ?>
					<div class="col-md-8">
						<?php if ($description) { ?>
				...
				<div class="category-list">
					...
					<div id="subcategory">
						<ul class="jcarousel-skin-opencart">
							<?php foreach ($kuler->getSubCategories($this->request->get['path'], $categories) as $sub_category) { ?>
								<li>
			...
			<?php if ($products) { ?>
				...
				<div class="product-list category-product-list">
					<?php foreach ($products as $product) { ?>
						...
						<?php } elseif ($column_left || $column_right) { ?>
							<?php $class = 'col-lg-4 col-md-4 col-sm-6'; ?>
						<?php } else { ?>
...
<script type="text/javascript"><!--
	...
	if (view) {
		...
		display(<?php echo json_encode($kuler->getSkinOption('display_product_mode')); ?>);
	}

	<?php if ($kuler->getSkinOption('sub_categories_image_carousel')) { ?>
	$(function () {
		$('#subcategory ul').jcarousel({
			vertical: false,
			visible: 4,
			scroll: 40
		});
