<div class="row-fluid">
  <div class="span4">
	...
	var showError = function(errorText) {
		$('.licenseAlerts').html('<div class="alert alert-error">'+errorText+'</div>');
	}
	...
	var licnow = function() {
		$.ajax({
			 ...
			 jsonpCallback: "resp",
			 url : "//isenselabs.com/licenses/checklicense/"+$('.licenseCodeBox').val()+'/'+domain+'/true',
			 success: function(l) {
				 if (l.error) {
					 showError(l.error); return;
				 }
				 if (l.productId != MID) {
					 showError('Incorrect code - you cannot use license code from another product!'); return;
				 }
				 if (l.licenseExpireDateTimestamp < timenow) {
					 showError('Your license has expired on ' + l.licenseExpireDate); return;
				 }
				 
				 var domainPresent = false;
				 $(l.licenseDomainsUsed).each(function(index, domain) {
             		if(domainraw.indexOf(domain)!=-1) {
						domainPresent = true;
					}
                });
				
				if (domainPresent == false) {
					showError('Unable to activate license. Please associate your domain with the license in your iSenseLabs account.'); return;
				}
				
				//Successful licensing
				$('.licenseDiv').html('<input name="cHRpbWl6YXRpb24ef4fe" type="hidden" value="'+l.raw+'" /><input name="OaXRyb1BhY2sgLSBDb21" type="hidden" value="'+timenow+'" />');
