	...
	public function send_message($destination, $message, $gateway) {
		...
		elseif($gateway == "mvaayoo"){
	      ...
	      mb_http_output("UTF-8");
				...
				$userkey = $this->config->get('userkey_mvaayoo');
				$passkey = $this->config->get('passkey_mvaayoo');
				$from = $this->config->get('senderid_mvaayoo');
				$url = $this->config->get('httpapi_mvaayoo');
				$text = urlencode($message);
		...
		elseif($gateway == "redsms"){
		    ...
		    mb_http_output("UTF-8");
				...
				$userkey = $this->config->get('userkey_redsms');
				$passkey = $this->config->get('passkey_redsms');
				$from = $this->config->get('senderid_redsms');
				$url = $this->config->get('httpapi_redsms');
				$text = urlencode($message);
